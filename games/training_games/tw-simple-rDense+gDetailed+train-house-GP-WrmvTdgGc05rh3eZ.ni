...
The kitchen part 0 is some text that varies. The kitchen part 0 is "You find yourself in a kitchen. A typical one. Let's see what's in here.

 You make out a refrigerator. Huh, weird.[if c_2 is open and there is something in the c_2] The refrigerator contains [a list of things in the c_2].[end if]".
...
The kitchen part 2 is some text that varies. The kitchen part 2 is " You rest your hand against a wall, but you miss the wall and fall onto a counter. [if there is something on the s_1]On the counter you see [a list of things on the s_1].[end if]".
...
The kitchen part 4 is some text that varies. The kitchen part 4 is " You can see a stove. The stove is typical.[if there is something on the s_2] On the stove you can see [a list of things on the s_2]. Classic TextWorld.[end if]".
...
The kitchen part 6 is some text that varies. The kitchen part 6 is " Look out! It's a- oh, never mind, it's just a kitchen island. What a coincidence, weren't you just thinking about a kitchen island? [if there is something on the s_3]You see [a list of things on the s_3] on the kitchen island. There's something strange about this being here, but you can't put your finger on it.[end if]".
...
The bedroom part 0 is some text that varies. The bedroom part 0 is "You have come into the most usual of all possible bedrooms.

 You can see [if c_0 is locked]a locked[else if c_0 is open]an opened[otherwise]a closed[end if]".
The bedroom part 1 is some text that varies. The bedroom part 1 is " chest drawer.[if c_0 is open and there is something in the c_0] The chest drawer contains [a list of things in the c_0]. Classic TextWorld.[end if]".
...
The bedroom part 3 is some text that varies. The bedroom part 3 is " Were you looking for an antique trunk? Because look over there, it's an antique trunk. Wow, isn't TextWorld just the best?[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1].[end if]".
...
The bedroom part 5 is some text that varies. The bedroom part 5 is " Were you looking for a king-size bed? Because look over there, it's a king-size bed. [if there is something on the s_0]On the king-size bed you can make out [a list of things on the s_0]. Hmmm... what else, what else?[end if]".
The bedroom part 6 is some text that varies. The bedroom part 6 is "[if there is nothing on the s_0]However, the king-size bed, like an empty king-size bed, has nothing on it. Hm. Oh well[end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "You are in a backyard. An usual kind of place.
 ...
 Hey, want to see a set of chairs? Look over there, a set of chairs. The set of chairs is ordinary.[if there is something on the s_10] On the set of chairs you make out [a list of things on the s_10].[end if]".
The backyard part 1 is some text that varies. The backyard part 1 is "[if there is nothing on the s_10] But the thing hasn't got anything on it. What, you think everything in TextWorld should have stuff on it?[end if]".
The backyard part 2 is some text that varies. The backyard part 2 is " You see a bbq. The bbq is typical.[if there is something on the s_8] On the bbq you can see [a list of things on the s_8].[end if]".
...
The backyard part 4 is some text that varies. The backyard part 4 is " As if things weren't amazing enough already, you can even see a patio table. [if there is something on the s_9]You see [a list of things on the s_9] on the patio table.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "If you're wondering why everything seems so standard all of a sudden, it's because you've just sauntered into the living room. You begin to take stock of what's here.

 You see a couch! The couch is usual.[if there is something on the s_5] On the couch you can make out [a list of things on the s_5].[end if]".
...
The living room part 4 is some text that varies. The living room part 4 is " You make out a tv. [if there is something on the s_7]You see [a list of things on the s_7] on the tv. Hmmm... what else, what else?[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7]But the thing is empty. Silly tv, silly, empty, good for nothing tv.[end if]".
...
The bathroom part 0 is some text that varies. The bathroom part 0 is "You are in a bathroom. A typical kind of place. The room is well lit.
...
The bathroom part 2 is some text that varies. The bathroom part 2 is " You bend down to tie your shoe. When you stand up, you notice a bath.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4]. Does this look like anything mentioned in the instructions?[end if]".
...
The bathroom part 4 is some text that varies. The bathroom part 4 is " You can make out a sink. Now why would someone leave that there? The sink is standard.[if there is something on the s_4] On the sink you make out [a list of things on the s_4].[end if]".
...
The d_0 is locked.
The description of d_1 is "it's a sturdy screen door [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The f_3 is in r_5.
The f_3 is edible.
The description of f_4 is "The bell pepper looks tasty.".
...
The description of f_2 is "The tomato plant looks appetizing.".
...
The objective part 0 is some text that varies. The objective part 0 is "I hope you're ready to go into rooms and interact with objects, because you've just entered TextWorld! Here is how to play! First, it would be fantastic if you could make absolutely sure that the ches".
The objective part 1 is some text that varies. The objective part 1 is "t drawer inside the bedroom is ajar. And then, take the old key from the chest drawer. With the old key, check that the wooden door inside the bedroom is unlocked with the old key. And then, make sure".
The objective part 2 is some text that varies. The objective part 2 is " that the wooden door is ajar. After opening the wooden door, try to move east. And then, ensure that the refrigerator in the kitchen is open. And then, retrieve the milk from the refrigerator within ".
The objective part 3 is some text that varies. The objective part 3 is "the kitchen. Then, put the milk on the stove in the kitchen. Got that? Good!".
