...
The kitchen part 2 is some text that varies. The kitchen part 2 is " You make out a counter. [if there is something on the s_1]On the counter you make out [a list of things on the s_1].[end if]".
The kitchen part 3 is some text that varies. The kitchen part 3 is "[if there is nothing on the s_1]But oh no! there's nothing on this piece of garbage. Aw, here you were, all excited for there to be things on it![end if]".
The kitchen part 4 is some text that varies. The kitchen part 4 is " You make out a stove. You wonder idly who left that here. The stove is normal.[if there is something on the s_2] On the stove you make out [a list of things on the s_2].[end if]".
The kitchen part 5 is some text that varies. The kitchen part 5 is "[if there is nothing on the s_2] But the thing is empty. Hm. Oh well[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is " You see a kitchen island. [if there is something on the s_3]You see [a list of things on the s_3] on the kitchen island. Hmmm... what else, what else?[end if]".
...
The bedroom part 4 is some text that varies. The bedroom part 4 is " antique trunk.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1]![end if]".
...
The bedroom part 6 is some text that varies. The bedroom part 6 is " Were you looking for a king-size bed? Because look over there, it's a king-size bed. [if there is something on the s_0]You see [a list of things on the s_0] on the king-size bed.[end if]".
The bedroom part 7 is some text that varies. The bedroom part 7 is "[if there is nothing on the s_0]However, the king-size bed, like an empty king-size bed, has nothing on it. Aw, here you were, all excited for there to be things on it![end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "You find yourself in a backyard. A typical kind of place.
 ...
 You lean against the wall, inadvertently pressing a secret button. The wall opens up to reveal a set of chairs. The set of chairs is ordinary.[if there is something on the s_10] On the set of chairs you can make out [a list of things on the s_10].[end if]".
The backyard part 1 is some text that varies. The backyard part 1 is "[if there is nothing on the s_10] Unfortunately, there isn't a thing on it. Hm. Oh well[end if]".
The backyard part 2 is some text that varies. The backyard part 2 is " You make out a bbq. The bbq is typical.[if there is something on the s_8] On the bbq you see [a list of things on the s_8].[end if]".
...
The backyard part 4 is some text that varies. The backyard part 4 is " You lean against the wall, inadvertently pressing a secret button. The wall opens up to reveal a patio table. [if there is something on the s_9]You see [a list of things on the s_9] on the patio table.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "You've entered a living room.

 What's that over there? It looks like it's a couch. The couch is ordinary.[if there is something on the s_5] On the couch you see [a list of things on the s_5].[end if]".
The living room part 1 is some text that varies. The living room part 1 is "[if there is nothing on the s_5] But oh no! there's nothing on this piece of junk.[end if]".
The living room part 2 is some text that varies. The living room part 2 is " You hear a noise behind you and spin around, but you can't see anything other than a low table. Wow, isn't TextWorld just the best? [if there is something on the s_6]On the low table you can see [a list of things on the s_6].[end if]".
The living room part 3 is some text that varies. The living room part 3 is "[if there is nothing on the s_6]Looks like someone's already been here and taken everything off it, though. This always happens![end if]".
The living room part 4 is some text that varies. The living room part 4 is " Look over there! a tv. [if there is something on the s_7]On the tv you make out [a list of things on the s_7].[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7]However, the tv, like an empty tv, has nothing on it. Oh! Why couldn't there just be stuff on it?[end if]".
...
The bathroom part 1 is some text that varies. The bathroom part 1 is " toilet here.[if c_3 is open and there is something in the c_3] The toilet contains [a list of things in the c_3].[end if]".
...
The bathroom part 4 is some text that varies. The bathroom part 4 is " bath.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4]. You wonder idly who left that here.[end if]".
...
The bathroom part 6 is some text that varies. The bathroom part 6 is " You can see a sink. What a coincidence, weren't you just thinking about a sink? The sink is standard.[if there is something on the s_4] On the sink you see [a list of things on the s_4].[end if]".
The bathroom part 7 is some text that varies. The bathroom part 7 is "[if there is nothing on the s_4] Unfortunately, there isn't a thing on it.[end if]".
...
The description of d_0 is "it's a grand wooden door [if open]You can see inside it.[else if closed]You can't see inside it because the lid's in your way.[otherwise]There is a lock on it.[end if]".
The printed name of d_0 is "wooden door".
Understand "wooden door" as d_0.
Understand "wooden" as d_0.
Understand "door" as d_0.
The d_0 is locked.
The description of d_1 is "The screen door looks stuffy. [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The f_4 is in r_5.
The f_4 is edible.
The description of f_5 is "that's an ordinary bell pepper!".
...
The s_9 is in r_4.
The description of f_0 is "The lettuce looks delicious.".
The printed name of f_0 is "lettuce".
Understand "lettuce" as f_0.
The f_0 is edible.
The f_0 is in the c_2.
The description of f_1 is "The apple looks tantalizing.".
...
The description of k_0 is "The metal of the old key is polished.".
...
The objective part 0 is some text that varies. The objective part 0 is "I hope you're ready to go into rooms and interact with objects, because you've just entered TextWorld! First thing I need you to do is to make absolutely sure that the antique trunk is ajar. Then, pic".
The objective part 1 is some text that varies. The objective part 1 is "k up the old key from the antique trunk in the bedroom. Once you have picked up the old key, unlock the wooden door. Then, doublecheck that the wooden door is wide open. And then, attempt to go to the".
The objective part 2 is some text that varies. The objective part 2 is " east. Then, make it so that the screen door is opened. Then, try to move east. And then, go south. And then, recover the half of a bag of chips from the floor of the garden. Then, try to travel north".
The objective part 3 is some text that varies. The objective part 3 is ". Then, try to travel west. And then, place the half of a bag of chips on the stove within the kitchen. Got that? Good!".
