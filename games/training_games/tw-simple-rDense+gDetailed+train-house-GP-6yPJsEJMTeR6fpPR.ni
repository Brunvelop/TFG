...
The kitchen part 0 is some text that varies. The kitchen part 0 is "Ah, the kitchen. This is some kind of kitchen, really great typical vibes in this place, a wonderful typical atmosphere.
...
The kitchen part 3 is some text that varies. The kitchen part 3 is " You lean against the wall, inadvertently pressing a secret button. The wall opens up to reveal a counter. [if there is something on the s_1]You see [a list of things on the s_1] on the counter.[end if]".
...
The kitchen part 5 is some text that varies. The kitchen part 5 is " You make out a stove. The stove is ordinary.[if there is something on the s_2] On the stove you can make out [a list of things on the s_2].[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is "[if there is nothing on the s_2] But oh no! there's nothing on this piece of garbage.[end if]".
The kitchen part 7 is some text that varies. The kitchen part 7 is " You can see a kitchen island. [if there is something on the s_3]You see [a list of things on the s_3] on the kitchen island.[end if]".
The kitchen part 8 is some text that varies. The kitchen part 8 is "[if there is nothing on the s_3]But the thing is empty. Aw, here you were, all excited for there to be things on it![end if]".
...
The kitchen part 11 is some text that varies. The kitchen part 11 is " wooden door leading west. You need an unguarded exit? You should try going north. You need an unguarded exit? You should try going south.".
...
The bedroom part 4 is some text that varies. The bedroom part 4 is " standard looking antique trunk right there by you.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1].[end if]".
...
The bedroom part 6 is some text that varies. The bedroom part 6 is " You see a king-size bed. [if there is something on the s_0]You see [a list of things on the s_0] on the king-size bed. Classic TextWorld.[end if]".
The bedroom part 7 is some text that varies. The bedroom part 7 is "[if there is nothing on the s_0]But the thing hasn't got anything on it.[end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "You are in a backyard. A standard kind of place. You decide to start listing off everything you see in the room, as if you were in a text adventure.

 You rest your hand against a wall, but you miss the wall and fall onto a set of chairs. Make a note of this, you might have to put stuff on or in it later on. The set of chairs is typical.[if there is something on the s_10] On the set of chairs you can make out [a list of things on the s_10].[end if]".
The backyard part 1 is some text that varies. The backyard part 1 is "[if there is nothing on the s_10] But the thing is empty, unfortunately.[end if]".
The backyard part 2 is some text that varies. The backyard part 2 is " You make out a bbq. You shudder, but continue examining the bbq. The bbq is normal.[if there is something on the s_8] On the bbq you make out [a list of things on the s_8].[end if]".
...
The backyard part 4 is some text that varies. The backyard part 4 is " You lean against the wall, inadvertently pressing a secret button. The wall opens up to reveal a patio table. The patio table is ordinary.[if there is something on the s_9] On the patio table you make out [a list of things on the s_9].[end if]".
The backyard part 5 is some text that varies. The backyard part 5 is "[if there is nothing on the s_9] But the thing is empty, unfortunately. You move on, clearly upset with TextWorld.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "Look around you. Take it all in. It's not every day someone gets to be in a living room.

 You see a couch. [if there is something on the s_5]You see [a list of things on the s_5] on the couch.[end if]".
...
The living room part 2 is some text that varies. The living room part 2 is " You make out a low table. The low table is usual.[if there is something on the s_6] On the low table you can see [a list of things on the s_6]. You shudder, but continue examining the room.[end if]".
The living room part 3 is some text that varies. The living room part 3 is "[if there is nothing on the s_6] But there isn't a thing on it. You move on, clearly depressed by TextWorld.[end if]".
The living room part 4 is some text that varies. The living room part 4 is " You can see a tv. The tv is standard.[if there is something on the s_7] On the tv you can see [a list of things on the s_7].[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7] However, the tv, like an empty tv, has nothing on it. This always happens, here in TextWorld![end if]".
...
The bathroom part 0 is some text that varies. The bathroom part 0 is "You've just walked into a bathroom.
 ...
 You make out a toilet.[if c_3 is open and there is something in the c_3] The toilet contains [a list of things in the c_3], so there's that.[end if]".
...
The bathroom part 2 is some text that varies. The bathroom part 2 is " As if things weren't amazing enough already, you can even see a bath.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4].[end if]".
The bathroom part 3 is some text that varies. The bathroom part 3 is "[if c_4 is open and the c_4 contains nothing] The bath is empty! This is the worst thing that could possibly happen, ever![end if]".
The bathroom part 4 is some text that varies. The bathroom part 4 is " You make out a sink. [if there is something on the s_4]You see [a list of things on the s_4] on the sink.[end if]".
The bathroom part 5 is some text that varies. The bathroom part 5 is "[if there is nothing on the s_4]However, the sink, like an empty sink, has nothing on it.[end if]".
...
The description of d_0 is "The wooden door looks well-built. [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The c_2 is in r_1.
The c_2 is closed.
The description of c_3 is "The toilet looks strong, and impossible to crack. [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The f_4 is in r_5.
The f_4 is edible.
The description of f_5 is "that's a standard tomato plant!".
...
The f_5 is in r_5.
The f_5 is edible.
The description of o_3 is "The shovel is cheap looking.".
The printed name of o_3 is "shovel".
Understand "shovel" as o_3.
The o_3 is in r_5.
The description of s_0 is "The king-size bed is reliable.".
...
The s_10 is in r_4.
The description of s_2 is "The stove is solidly built.".
...
The s_7 is in r_2.
The description of s_8 is "The bbq is an unstable piece of garbage.".
...
The s_9 is in r_4.
The description of f_0 is "that's an ordinary lettuce!".
The printed name of f_0 is "lettuce".
Understand "lettuce" as f_0.
The f_0 is edible.
The f_0 is in the c_2.
The description of f_1 is "that's a typical bell pepper!".
...
The printed name of f_2 is "half of a bag of chips".
Understand "half of a bag of chips" as f_2.
Understand "half" as f_2.
Understand "a" as f_2.
Understand "bag" as f_2.
Understand "chips" as f_2.
The f_2 is edible.
The f_2 is on the s_5.
The description of k_0 is "The old key looks useful".
...
The description of o_1 is "The soap bar is expensive looking.".
...
The description of o_0 is "The toothbrush appears out of place here".
The printed name of o_0 is "toothbrush".
Understand "toothbrush" as o_0.
The o_0 is on the s_4.
The description of o_2 is "The remote seems well matched to everything else here".
...
Test quest8_0 with "open antique trunk / take old key from antique trunk / unlock wooden door with old key / open wooden door / go east / open screen door / go east / go south / take tomato plant"
...
Test quest9_0 with "open antique trunk / take old key from antique trunk / unlock wooden door with old key / open wooden door / go east / open screen door / go east / go south / take tomato plant / go north / go west / put tomato plant on stove"
...
The objective part 0 is some text that varies. The objective part 0 is "You are now playing a exciting game of TextWorld! Here is your task for today. First, it would be good if you could ensure that the antique trunk within the bedroom is open. If you have opened the ant".
The objective part 1 is some text that varies. The objective part 1 is "ique trunk, recover the old key from the antique trunk. And then, check that the wooden door is unlocked with the old key. After you have unlocked the wooden door, open the wooden door. And then, vent".
The objective part 2 is some text that varies. The objective part 2 is "ure east. And then, open the screen door within the kitchen. And then, take a trip east. And then, try to go to the south. With that over with, pick-up the tomato plant from the floor of the garden. A".
The objective part 3 is some text that varies. The objective part 3 is "fter picking up the tomato plant, attempt to move north. Once you manage that, venture west. And then, put the tomato plant on the stove. Got that? Good!".
