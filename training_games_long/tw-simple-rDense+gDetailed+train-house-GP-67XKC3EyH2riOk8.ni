...
The kitchen part 1 is some text that varies. The kitchen part 1 is " refrigerator nearby.[if c_2 is open and there is something in the c_2] The refrigerator contains [a list of things in the c_2]. You wonder idly who left that here.[end if]".
...
The kitchen part 3 is some text that varies. The kitchen part 3 is " You can see a counter. Make a note of this, you might have to put stuff on or in it later on. The counter is normal.[if there is something on the s_1] On the counter you see [a list of things on the s_1]. Hmmm... what else, what else?[end if]".
The kitchen part 4 is some text that varies. The kitchen part 4 is "[if there is nothing on the s_1] Unfortunately, there isn't a thing on it. Aw, here you were, all excited for there to be things on it![end if]".
The kitchen part 5 is some text that varies. The kitchen part 5 is " You hear a noise behind you and spin around, but you can't see anything other than a stove. [if there is something on the s_2]You see [a list of things on the s_2] on the stove. Something scurries by right in the corner of your eye. Probably nothing.[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is "[if there is nothing on the s_2]But the thing is empty. Aw, here you were, all excited for there to be things on it![end if]".
The kitchen part 7 is some text that varies. The kitchen part 7 is " You see a kitchen island. The kitchen island is usual.[if there is something on the s_3] On the kitchen island you can see [a list of things on the s_3]. Something scurries by right in the corner of your eye. Probably nothing.[end if]".
...
The kitchen part 11 is some text that varies. The kitchen part 11 is " wooden door leading west. There is an unblocked exit to the north. You need an unguarded exit? You should try going south.".
...
The bedroom part 3 is some text that varies. The bedroom part 3 is " Look out! It's a- oh, never mind, it's just an antique trunk. Hmm. You always thought you'd be more excited to see an antique trunk in a room.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1].[end if]".
...
The bedroom part 5 is some text that varies. The bedroom part 5 is " You can make out a king-size bed. [if there is something on the s_0]You see [a list of things on the s_0] on the king-size bed.[end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "If you're wondering why everything seems so typical all of a sudden, it's because you've just walked into the backyard. You decide to start listing off everything you see in the room, as if you were in a text adventure.

 You make out a set of chairs. The set of chairs is usual.[if there is something on the s_10] On the set of chairs you can see [a list of things on the s_10].[end if]".
...
The backyard part 2 is some text that varies. The backyard part 2 is " You make out a bbq. The bbq is ordinary.[if there is something on the s_8] On the bbq you can see [a list of things on the s_8]. There's something strange about this being here, but you can't put your finger on it.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "You've entered an ordinary room. Your mind races to think of what kind of room would be ordinary. And then it hits you. Of course. You're in the living room.

 Oh wow! Is that what I think it is? It is! It's a couch. [if there is something on the s_5]On the couch you can see [a list of things on the s_5].[end if]".
The living room part 1 is some text that varies. The living room part 1 is "[if there is nothing on the s_5]But oh no! there's nothing on this piece of trash. Oh! Why couldn't there just be stuff on it?[end if]".
The living room part 2 is some text that varies. The living room part 2 is " Oh wow! Is that what I think it is? It is! It's a low table. [if there is something on the s_6]On the low table you make out [a list of things on the s_6].[end if]".
...
The living room part 4 is some text that varies. The living room part 4 is " You make out a tv. [if there is something on the s_7]You see [a list of things on the s_7] on the tv, so there's that.[end if]".
...
The bathroom part 1 is some text that varies. The bathroom part 1 is " toilet right there by you.[if c_3 is open and there is something in the c_3] The toilet contains [a list of things in the c_3]. The light flickers for a second, but nothing else happens.[end if]".
...
The bathroom part 3 is some text that varies. The bathroom part 3 is " You can make out [if c_4 is locked]a locked[else if c_4 is open]an opened[otherwise]a closed[end if]".
...
The bathroom part 6 is some text that varies. The bathroom part 6 is " You scan the room, seeing a sink. [if there is something on the s_4]On the sink you can make out [a list of things on the s_4]. You idly wonder how they came up with the name TextWorld for this place. It's pretty fitting.[end if]".
The bathroom part 7 is some text that varies. The bathroom part 7 is "[if there is nothing on the s_4]But the thing is empty, unfortunately.[end if]".
...
The c_4 is in r_3.
The c_4 is closed.
The description of f_3 is "that's a normal lettuce!".
...
The description of o_1 is "The soap bar looks out of place here".
...
The description of o_2 is "The remote would seem to be well matched to everything else here".
...
The objective part 0 is some text that varies. The objective part 0 is "I hope you're ready to go into rooms and interact with objects, because you've just entered TextWorld! Here is your task for today. First step, make absolutely sure that the chest drawer in the bedroo".
The objective part 1 is some text that varies. The objective part 1 is "m is wide open. After that, pick up the old key from the chest drawer. And then, unlock the wooden door. And then, ensure that the wooden door inside the bedroom is open. After opening the wooden door".
The objective part 2 is some text that varies. The objective part 2 is ", take a trip east. Then, open the screen door. And then, move east. And then, attempt to travel south. Once you get through with that, retrieve the bell pepper from the floor of the garden. And then,".
The objective part 3 is some text that varies. The objective part 3 is " make an attempt to take a trip north. Following that, go west. With that accomplished, place the bell pepper on the stove. Got that? Good!".
