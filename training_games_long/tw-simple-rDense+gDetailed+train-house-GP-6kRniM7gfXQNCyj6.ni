...
The kitchen part 1 is some text that varies. The kitchen part 1 is " refrigerator, which looks standard, is in the room.[if c_2 is open and there is something in the c_2] The refrigerator contains [a list of things in the c_2].[end if]".
...
The kitchen part 3 is some text that varies. The kitchen part 3 is " Oh, great. Here's a counter. [if there is something on the s_1]On the counter you see [a list of things on the s_1]. There's something strange about this being here, but you can't put your finger on it.[end if]".
The kitchen part 4 is some text that varies. The kitchen part 4 is "[if there is nothing on the s_1]But the thing hasn't got anything on it. Hopefully, this discovery doesn't ruin your TextWorld experience![end if]".
The kitchen part 5 is some text that varies. The kitchen part 5 is " You can make out a stove. The stove is usual.[if there is something on the s_2] On the stove you make out [a list of things on the s_2]. You shudder, but continue examining the room.[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is "[if there is nothing on the s_2] However, the stove, like an empty stove, has nothing on it.[end if]".
The kitchen part 7 is some text that varies. The kitchen part 7 is " You can make out a kitchen island. The kitchen island is ordinary.[if there is something on the s_3] On the kitchen island you can make out [a list of things on the s_3]. Hmmm... what else, what else?[end if]".
...
The kitchen part 11 is some text that varies. The kitchen part 11 is " wooden door leading west. You need an unguarded exit? You should try going north. There is an exit to the south. Don't worry, it is unblocked.".
...
The bedroom part 1 is some text that varies. The bedroom part 1 is " chest drawer.[if c_0 is open and there is something in the c_0] The chest drawer contains [a list of things in the c_0]. The light flickers for a second, but nothing else happens.[end if]".
...
The bedroom part 3 is some text that varies. The bedroom part 3 is " You rest your hand against a wall, but you miss the wall and fall onto an antique trunk.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1].[end if]".
...
The bedroom part 5 is some text that varies. The bedroom part 5 is " You can make out a king-size bed. [if there is something on the s_0]On the king-size bed you can see [a list of things on the s_0].[end if]".
The bedroom part 6 is some text that varies. The bedroom part 6 is "[if there is nothing on the s_0]But oh no! there's nothing on this piece of garbage.[end if]".
...
The backyard part 1 is some text that varies. The backyard part 1 is "[if there is nothing on the s_10]But oh no! there's nothing on this piece of junk. Sometimes, just sometimes, TextWorld can just be the worst.[end if]".
The backyard part 2 is some text that varies. The backyard part 2 is " You can see a bbq. [if there is something on the s_8]You see [a list of things on the s_8] on the bbq.[end if]".
...
The backyard part 4 is some text that varies. The backyard part 4 is " Oh wow! Is that what I think it is? It is! It's a patio table. [if there is something on the s_9]On the patio table you make out [a list of things on the s_9]. Huh, weird.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "You've entered a living room. You decide to start listing off everything you see in the room, as if you were in a text adventure.

 You can see a couch. The couch is typical.[if there is something on the s_5] On the couch you see [a list of things on the s_5]. Suddenly, you bump your head on the ceiling, but it's not such a bad bump that it's going to prevent you from looking at objects and even things.[end if]".
The living room part 1 is some text that varies. The living room part 1 is "[if there is nothing on the s_5] Unfortunately, there isn't a thing on it. This always happens, here in TextWorld![end if]".
The living room part 2 is some text that varies. The living room part 2 is " You see a low table. I guess it's true what they say, if you're looking for a low table, go to TextWorld. The low table is normal.[if there is something on the s_6] On the low table you make out [a list of things on the s_6].[end if]".
...
The living room part 4 is some text that varies. The living room part 4 is " You make out a tv. [if there is something on the s_7]On the tv you make out [a list of things on the s_7].[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7]Looks like someone's already been here and taken everything off it, though. You make a mental note to not get your hopes up the next time you see a tv in a room.[end if]".
...
The bathroom part 1 is some text that varies. The bathroom part 1 is " toilet close by.[if c_3 is open and there is something in the c_3] The toilet contains [a list of things in the c_3]. Huh, weird.[end if]".
...
The bathroom part 6 is some text that varies. The bathroom part 6 is " You can make out a sink. [if there is something on the s_4]On the sink you can make out [a list of things on the s_4].[end if]".
The bathroom part 7 is some text that varies. The bathroom part 7 is "[if there is nothing on the s_4]But the thing is empty.[end if]".
...
The description of d_0 is "The wooden door looks imposing. [if open]You can see inside it.[else if closed]You can't see inside it because the lid's in your way.[otherwise]There is a lock on it.[end if]".
The printed name of d_0 is "wooden door".
Understand "wooden door" as d_0.
Understand "wooden" as d_0.
Understand "door" as d_0.
The d_0 is locked.
The description of d_1 is "it's a grand screen door [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The s_9 is in r_4.
The description of f_0 is "The milk looks heavenly.".
...
The description of o_1 is "The soap bar appears to be to fit in here".
...
The objective part 0 is some text that varies. The objective part 0 is "Get ready to pick stuff up and put it in places, because you've just entered TextWorld! Here is your task for today. First off, doublecheck that the antique trunk inside the bedroom is open. Once you ".
The objective part 1 is some text that varies. The objective part 1 is "have opened the antique trunk, retrieve the old key from the antique trunk. And then, unlock the wooden door with the old key. Having unlocked the wooden door, make it so that the wooden door is open.".
The objective part 2 is some text that varies. The objective part 2 is " After that, move east. With that done, open the screen door. And then, venture east. Then, make an attempt to go south. And then, pick-up the half of a bag of chips from the floor of the garden. And ".
The objective part 3 is some text that varies. The objective part 3 is "then, take a trip north. With that accomplished, attempt to go to the west. And then, put the half of a bag of chips on the stove. Got that? Good!".
