...
The kitchen part 0 is some text that varies. The kitchen part 0 is "I am stoked to announce that you are now in the kitchen. The room seems oddly familiar, as though it were only superficially different from the other rooms in the building.

 You see a refrigerator.[if c_2 is open and there is something in the c_2] The refrigerator contains [a list of things in the c_2].[end if]".
...
The kitchen part 2 is some text that varies. The kitchen part 2 is " You make out a counter. You shudder, but continue examining the counter. [if there is something on the s_1]On the counter you can make out [a list of things on the s_1].[end if]".
The kitchen part 3 is some text that varies. The kitchen part 3 is "[if there is nothing on the s_1]Unfortunately, there isn't a thing on it.[end if]".
The kitchen part 4 is some text that varies. The kitchen part 4 is " You see a stove. The stove is standard.[if there is something on the s_2] On the stove you can make out [a list of things on the s_2].[end if]".
The kitchen part 5 is some text that varies. The kitchen part 5 is "[if there is nothing on the s_2] However, the stove, like an empty stove, has nothing on it.[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is " You can see a kitchen island. The kitchen island is usual.[if there is something on the s_3] On the kitchen island you make out [a list of things on the s_3].[end if]".
The kitchen part 7 is some text that varies. The kitchen part 7 is "[if there is nothing on the s_3] Unfortunately, there isn't a thing on it. It would have been so cool if there was stuff on the kitchen island.[end if]".
...
The kitchen part 10 is some text that varies. The kitchen part 10 is " wooden door leading west. You need an unguarded exit? You should try going north. You need an unblocked exit? You should try going south.".
...
The bedroom part 1 is some text that varies. The bedroom part 1 is " chest drawer.[if c_0 is open and there is something in the c_0] The chest drawer contains [a list of things in the c_0]. Suddenly, you bump your head on the ceiling, but it's not such a bad bump that it's going to prevent you from looking at objects and even things.[end if]".
...
The bedroom part 3 is some text that varies. The bedroom part 3 is " You can see an antique trunk.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1].[end if]".
...
The bedroom part 6 is some text that varies. The bedroom part 6 is "[if there is nothing on the s_0]But the thing is empty, unfortunately.[end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "Well, here we are in a backyard. You can barely contain your excitement.
 ...
 What's that over there? It looks like it's a set of chairs. [if there is something on the s_10]On the set of chairs you can make out [a list of things on the s_10]. Hmmm... what else, what else?[end if]".
The backyard part 1 is some text that varies. The backyard part 1 is "[if there is nothing on the s_10]However, the set of chairs, like an empty set of chairs, has nothing on it. It would have been so cool if there was stuff on the set of chairs.[end if]".
The backyard part 2 is some text that varies. The backyard part 2 is " You smell a wretched smell, and follow it to a bbq. The bbq is ordinary.[if there is something on the s_8] On the bbq you can make out [a list of things on the s_8].[end if]".
The backyard part 3 is some text that varies. The backyard part 3 is "[if there is nothing on the s_8] Looks like someone's already been here and taken everything off it, though. Silly bbq, silly, empty, good for nothing bbq.[end if]".
The backyard part 4 is some text that varies. The backyard part 4 is " As if things weren't amazing enough already, you can even see a patio table. The patio table is usual.[if there is something on the s_9] On the patio table you make out [a list of things on the s_9].[end if]".
The backyard part 5 is some text that varies. The backyard part 5 is "[if there is nothing on the s_9] But oh no! there's nothing on this piece of junk. Aw, here you were, all excited for there to be things on it![end if]".
The backyard part 6 is some text that varies. The backyard part 6 is "

 There is [if d_1 is open]an open[otherwise]a closed[end if]".
The backyard part 7 is some text that varies. The backyard part 7 is " screen door leading west. There is an exit to the south. Don't worry, it is unguarded.".
...
The living room part 0 is some text that varies. The living room part 0 is "Well how about that, you are in a place we're calling a living room.

 You scan the room for a couch, and you find a couch. Now why would someone leave that there? [if there is something on the s_5]You see [a list of things on the s_5] on the couch.[end if]".
The living room part 1 is some text that varies. The living room part 1 is "[if there is nothing on the s_5]But oh no! there's nothing on this piece of junk. This always happens, here in TextWorld![end if]".
The living room part 2 is some text that varies. The living room part 2 is " You see a low table. [if there is something on the s_6]On the low table you see [a list of things on the s_6].[end if]".
...
The living room part 4 is some text that varies. The living room part 4 is " You see a tv. [if there is something on the s_7]On the tv you see [a list of things on the s_7].[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7]However, the tv, like an empty tv, has nothing on it. Hm. Oh well[end if]".
...
The bathroom part 0 is some text that varies. The bathroom part 0 is "You find yourself in a bathroom. An ordinary one.
 ...
 Hey, want to see a toilet? Look over there, a toilet.[if c_3 is open and there is something in the c_3] The toilet contains [a list of things in the c_3].[end if]".
...
The bathroom part 3 is some text that varies. The bathroom part 3 is " bath.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4]. Does this look like anything mentioned in the instructions?[end if]".
...
The bathroom part 5 is some text that varies. The bathroom part 5 is " You rest your hand against a wall, but you miss the wall and fall onto a sink. The sink is typical.[if there is something on the s_4] On the sink you can see [a list of things on the s_4]. You shudder, but continue examining the room.[end if]".
...
The garden part 0 is some text that varies. The garden part 0 is "Guess what, you are in the place we're calling the garden. You decide to start listing off everything you see in the room, as if you were in a text adventure.
...
The description of d_0 is "it's a towering wooden door [if open]You can see inside it.[else if closed]You can't see inside it because the lid's in your way.[otherwise]There is a lock on it.[end if]".
The printed name of d_0 is "wooden door".
Understand "wooden door" as d_0.
Understand "wooden" as d_0.
Understand "door" as d_0.
The d_0 is locked.
The description of d_1 is "it's an ominous screen door [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The f_3 is in r_5.
The f_3 is edible.
The description of f_4 is "The bell pepper looks appealing.".
...
The f_4 is in r_5.
The f_4 is edible.
The description of f_5 is "The tomato plant looks delectable.".
...
The f_5 is in r_5.
The f_5 is edible.
The description of o_3 is "The shovel seems to fit in here".
...
The s_2 is in r_1.
The description of s_3 is "The kitchen island is an unstable piece of trash.".
...
The s_3 is in r_1.
The description of s_4 is "The sink is an unstable piece of garbage.".
...
The description of f_1 is "that's a typical half of a bag of chips!".
...
The description of o_1 is "The soap bar appears well matched to everything else here".
...
Test quest8_0 with "open antique trunk / take old key from antique trunk / unlock wooden door with old key / open wooden door / go east / open screen door / go east / go south / take bell pepper"
...
Test quest9_0 with "open antique trunk / take old key from antique trunk / unlock wooden door with old key / open wooden door / go east / open screen door / go east / go south / take bell pepper / go north / go west / put bell pepper on stove"
...
The objective part 0 is some text that varies. The objective part 0 is "Get ready to pick stuff up and put it in places, because you've just entered TextWorld! First off, if it's not too much trouble, I need you to make sure that the antique trunk is opened. And then, ret".
The objective part 1 is some text that varies. The objective part 1 is "rieve the old key from the antique trunk inside the bedroom. Then, unlock the wooden door within the bedroom. And then, open the wooden door. And then, try to go to the east. Following that, open the ".
The objective part 2 is some text that varies. The objective part 2 is "screen door. Then, try to venture east. And then, try to take a trip south. Then, lift the bell pepper from the floor of the garden. After picking up the bell pepper, make an attempt to go to the nort".
The objective part 3 is some text that varies. The objective part 3 is "h. Then, go to the west. And then, put the bell pepper on the stove in the kitchen. Alright, thanks!".
