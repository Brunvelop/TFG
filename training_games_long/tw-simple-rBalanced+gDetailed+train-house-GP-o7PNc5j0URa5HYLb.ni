...
The kitchen part 2 is some text that varies. The kitchen part 2 is " What's that over there? It looks like it's a counter. Why don't you take a picture of it, it'll last longer! [if there is something on the s_1]On the counter you can make out [a list of things on the s_1]. Huh, weird.[end if]".
...
The kitchen part 4 is some text that varies. The kitchen part 4 is " Look over there! a stove. [if there is something on the s_2]You see [a list of things on the s_2] on the stove.[end if]".
The kitchen part 5 is some text that varies. The kitchen part 5 is "[if there is nothing on the s_2]But oh no! there's nothing on this piece of trash.[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is " You hear a noise behind you and spin around, but you can't see anything other than a kitchen island. [if there is something on the s_3]On the kitchen island you see [a list of things on the s_3].[end if]".
...
The bedroom part 0 is some text that varies. The bedroom part 0 is "You've just sauntered into a bedroom.
 ...
 You can see a chest drawer. Wow, isn't TextWorld just the best?[if c_0 is open and there is something in the c_0] The chest drawer contains [a list of things in the c_0]. Now that's what I call TextWorld![end if]".
...
The bedroom part 2 is some text that varies. The bedroom part 2 is " You see an antique trunk.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1]. Something scurries by right in the corner of your eye. Probably nothing.[end if]".
...
The bedroom part 4 is some text that varies. The bedroom part 4 is " You rest your hand against a wall, but you miss the wall and fall onto a king-size bed. The king-size bed is usual.[if there is something on the s_0] On the king-size bed you see [a list of things on the s_0]. Suddenly, you bump your head on the ceiling, but it's not such a bad bump that it's going to prevent you from looking at objects and even things.[end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "You arrive in a backyard. An ordinary kind of place.
 ...
 You can see a set of chairs. The set of chairs is typical.[if there is something on the s_10] On the set of chairs you can make out [a list of things on the s_10].[end if]".
...
The backyard part 2 is some text that varies. The backyard part 2 is " You make out a bbq. The bbq is usual.[if there is something on the s_8] On the bbq you can make out [a list of things on the s_8].[end if]".
...
The backyard part 4 is some text that varies. The backyard part 4 is " You see a gleam over in a corner, where you can see a patio table. [if there is something on the s_9]On the patio table you can see [a list of things on the s_9], so there's that.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "You've just walked into a living room. You decide to just list off a complete list of everything you see in the room, because hey, why not?

 You can make out a couch. Wow, isn't TextWorld just the best? The couch is typical.[if there is something on the s_5] On the couch you can make out [a list of things on the s_5].[end if]".
The living room part 1 is some text that varies. The living room part 1 is "[if there is nothing on the s_5] The couch appears to be empty. Aw, here you were, all excited for there to be things on it![end if]".
The living room part 2 is some text that varies. The living room part 2 is " If you haven't noticed it already, there seems to be something there by the wall, it's a low table. [if there is something on the s_6]On the low table you can see [a list of things on the s_6].[end if]".
...
The living room part 4 is some text that varies. The living room part 4 is " You see a tv. What a coincidence, weren't you just thinking about a tv? The tv is standard.[if there is something on the s_7] On the tv you can make out [a list of things on the s_7].[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7] Looks like someone's already been here and taken everything off it, though. Oh! Why couldn't there just be stuff on it?[end if]".
...
The bathroom part 3 is some text that varies. The bathroom part 3 is " bath in the corner.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4].[end if]".
...
The bathroom part 5 is some text that varies. The bathroom part 5 is " Oh, great. Here's a sink. The sink is normal.[if there is something on the s_4] On the sink you can make out [a list of things on the s_4].[end if]".
The bathroom part 6 is some text that varies. The bathroom part 6 is "[if there is nothing on the s_4] But the thing is empty, unfortunately. Hm. Oh well[end if]".
...
The description of d_0 is "The wooden door looks solid. [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
...
The f_3 is in r_5.
The f_3 is edible.
The description of f_4 is "The milk looks appetizing.".
...
The description of o_1 is "The soap bar would seem to be to fit in here".
...
The objective part 0 is some text that varies. The objective part 0 is "Who's got a virtual machine and is about to play through an exciting round of TextWorld? You do! Here is your task for today. First, it would be fantastic if you could make sure that the antique trunk".
The objective part 1 is some text that varies. The objective part 1 is " within the bedroom is open. And then, retrieve the old key from the antique trunk within the bedroom. After taking the old key, make absolutely sure that the wooden door is unlocked. Then, make sure ".
The objective part 2 is some text that varies. The objective part 2 is "that the wooden door within the bedroom is wide open. Then, go to the east. Then, open the screen door. After that, venture east. Next, attempt to go to the south. And then, pick up the milk from the ".
The objective part 3 is some text that varies. The objective part 3 is "floor of the garden. If you can get your hands on the milk, travel north. With that over with, venture west. And then, rest the milk on the stove in the kitchen. That's it!".
