...
The kitchen part 0 is some text that varies. The kitchen part 0 is "Look at that placard! What does it say? It says Welcome to the kitchen? Well that's cool. The room seems oddly familiar, as though it were only superficially different from the other rooms in the building.

 You can see [if c_2 is locked]a locked[else if c_2 is open]an opened[otherwise]a closed[end if]".
The kitchen part 1 is some text that varies. The kitchen part 1 is " refrigerator close by.[if c_2 is open and there is something in the c_2] The refrigerator contains [a list of things in the c_2].[end if]".
...
The kitchen part 3 is some text that varies. The kitchen part 3 is " You can make out a counter. The counter is typical.[if there is something on the s_1] On the counter you can see [a list of things on the s_1].[end if]".
...
The kitchen part 5 is some text that varies. The kitchen part 5 is " You see a stove. [if there is something on the s_2]On the stove you can see [a list of things on the s_2].[end if]".
The kitchen part 6 is some text that varies. The kitchen part 6 is "[if there is nothing on the s_2]Unfortunately, there isn't a thing on it. Hm. Oh well[end if]".
The kitchen part 7 is some text that varies. The kitchen part 7 is " Were you looking for a kitchen island? Because look over there, it's a kitchen island. [if there is something on the s_3]On the kitchen island you can see [a list of things on the s_3].[end if]".
...
The kitchen part 11 is some text that varies. The kitchen part 11 is " wooden door leading west. You don't like doors? Why not try going north, that entranceway is unblocked. There is an unguarded exit to the south.".
...
The bedroom part 0 is some text that varies. The bedroom part 0 is "Okay, so you're in a bedroom, cool, but is it standard? You better believe it is.
 ...
 Hey, want to see a chest drawer? Look over there, a chest drawer.[if c_0 is open and there is something in the c_0] The chest drawer contains [a list of things in the c_0]. Suddenly, you bump your head on the ceiling, but it's not such a bad bump that it's going to prevent you from looking at objects and even things.[end if]".
...
The bedroom part 2 is some text that varies. The bedroom part 2 is " Oh, great. Here's an antique trunk.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1]. Wow, isn't TextWorld just the best?[end if]".
...
The bedroom part 4 is some text that varies. The bedroom part 4 is " You see a king-size bed. The king-size bed is typical.[if there is something on the s_0] On the king-size bed you see [a list of things on the s_0]. You can't wait to tell the folks at home about this![end if]".
...
The backyard part 0 is some text that varies. The backyard part 0 is "You've just sauntered into a backyard.
 ...
 You can make out a set of chairs. The set of chairs is ordinary.[if there is something on the s_10] On the set of chairs you can make out [a list of things on the s_10]. You idly wonder how they came up with the name TextWorld for this place. It's pretty fitting.[end if]".
The backyard part 1 is some text that varies. The backyard part 1 is "[if there is nothing on the s_10] But the thing is empty.[end if]".
The backyard part 2 is some text that varies. The backyard part 2 is " You see a bbq. [if there is something on the s_8]On the bbq you make out [a list of things on the s_8]. Classic TextWorld.[end if]".
The backyard part 3 is some text that varies. The backyard part 3 is "[if there is nothing on the s_8]The bbq appears to be empty. It would have been so cool if there was stuff on the bbq.[end if]".
The backyard part 4 is some text that varies. The backyard part 4 is " You see a patio table. The patio table is ordinary.[if there is something on the s_9] On the patio table you see [a list of things on the s_9]. Classic TextWorld.[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "Look at you, bigshot, walking into a living room like it isn't some huge deal. You begin to take stock of what's here.
...
The living room part 2 is some text that varies. The living room part 2 is " You make out a low table. The low table is typical.[if there is something on the s_6] On the low table you see [a list of things on the s_6].[end if]".
The living room part 3 is some text that varies. The living room part 3 is "[if there is nothing on the s_6] However, the low table, like an empty low table, has nothing on it. Oh! Why couldn't there just be stuff on it?[end if]".
The living room part 4 is some text that varies. The living room part 4 is " You see a gleam over in a corner, where you can see a tv. The tv is standard.[if there is something on the s_7] On the tv you see [a list of things on the s_7]. There's something strange about this being here, but you can't put your finger on it.[end if]".
The living room part 5 is some text that varies. The living room part 5 is "[if there is nothing on the s_7] But the thing is empty, unfortunately. What, you think everything in TextWorld should have stuff on it?[end if]".
...
The bathroom part 1 is some text that varies. The bathroom part 1 is " toilet.[if c_3 is open and there is something in the c_3] The toilet contains [a list of things in the c_3]. You can't wait to tell the folks at home about this![end if]".
...
The bathroom part 3 is some text that varies. The bathroom part 3 is " You make out a bath. Something scurries by right in the corner of your eye. Probably nothing.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4].[end if]".
...
The bathroom part 5 is some text that varies. The bathroom part 5 is " You scan the room, seeing a sink. You shudder, but continue examining the sink. [if there is something on the s_4]You see [a list of things on the s_4] on the sink. Something scurries by right in the corner of your eye. Probably nothing.[end if]".
The bathroom part 6 is some text that varies. The bathroom part 6 is "[if there is nothing on the s_4]But the thing is empty. Hm. Oh well[end if]".
...
The garden part 0 is some text that varies. The garden part 0 is "You arrive in a garden. A typical one. The room is well lit.
...
The description of d_0 is "it's a noble wooden door [if open]You can see inside it.[else if closed]You can't see inside it because the lid's in your way.[otherwise]There is a lock on it.[end if]".
...
The c_4 is in r_3.
The c_4 is closed.
The description of f_3 is "The milk looks savory.".
The printed name of f_3 is "milk".
Understand "milk" as f_3.
The f_3 is in r_5.
The f_3 is edible.
The description of f_4 is "The lettuce looks savory.".
...
The s_9 is in r_4.
The description of f_0 is "The bell pepper looks delicious.".
...
The objective part 0 is some text that varies. The objective part 0 is "Hey, thanks for coming over to the TextWorld today, there is something I need you to do for me. First off, ensure that the chest drawer is open. And then, take the old key from the chest drawer within".
The objective part 1 is some text that varies. The objective part 1 is " the bedroom. After stealing the old key, unlock the wooden door in the bedroom. Then, look and see that the wooden door inside the bedroom is open. And then, travel east. Okay, and then, ensure that ".
The objective part 2 is some text that varies. The objective part 2 is "the screen door is open. Then, go east. And then, head south. And then, pick-up the lettuce from the floor of the garden. After stealing the lettuce, make an effort to go to the north. Then, make an e".
The objective part 3 is some text that varies. The objective part 3 is "ffort to travel west. If you can do that, put the lettuce on the stove in the kitchen. Alright, thanks!".
