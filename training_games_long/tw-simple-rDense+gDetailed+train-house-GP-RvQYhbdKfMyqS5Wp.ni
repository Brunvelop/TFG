...
The kitchen part 1 is some text that varies. The kitchen part 1 is " refrigerator, which looks standard, is nearby.[if c_2 is open and there is something in the c_2] The refrigerator contains [a list of things in the c_2].[end if]".
...
The kitchen part 4 is some text that varies. The kitchen part 4 is "[if there is nothing on the s_1] The counter appears to be empty. Oh! Why couldn't there just be stuff on it?[end if]".
The kitchen part 5 is some text that varies. The kitchen part 5 is " You can see a stove. Make a note of this, you might have to put stuff on or in it later on. The stove is ordinary.[if there is something on the s_2] On the stove you see [a list of things on the s_2].[end if]".
...
The kitchen part 7 is some text that varies. The kitchen part 7 is " Oh, great. Here's a kitchen island. The kitchen island is usual.[if there is something on the s_3] On the kitchen island you can make out [a list of things on the s_3].[end if]".
The kitchen part 8 is some text that varies. The kitchen part 8 is "[if there is nothing on the s_3] But oh no! there's nothing on this piece of junk.[end if]".
...
The kitchen part 11 is some text that varies. The kitchen part 11 is " wooden door leading west. You don't like doors? Why not try going north, that entranceway is unblocked. You need an unblocked exit? You should try going south.".
...
The bedroom part 2 is some text that varies. The bedroom part 2 is " You rest your hand against a wall, but you miss the wall and fall onto an antique trunk.[if c_1 is open and there is something in the c_1] The antique trunk contains [a list of things in the c_1]. Wow, isn't TextWorld just the best?[end if]".
...
The bedroom part 4 is some text that varies. The bedroom part 4 is " You smell a hideous smell, and follow it to a king-size bed. [if there is something on the s_0]On the king-size bed you make out [a list of things on the s_0]. You shudder, but continue examining the room.[end if]".
...
The backyard part 2 is some text that varies. The backyard part 2 is " Were you looking for a bbq? Because look over there, it's a bbq. [if there is something on the s_8]You see [a list of things on the s_8] on the bbq.[end if]".
...
The backyard part 4 is some text that varies. The backyard part 4 is " You can see a patio table! [if there is something on the s_9]On the patio table you see [a list of things on the s_9]. Wow! Just like in the movies![end if]".
The backyard part 5 is some text that varies. The backyard part 5 is "[if there is nothing on the s_9]But there isn't a thing on it. Oh! Why couldn't there just be stuff on it?[end if]".
...
The living room part 0 is some text that varies. The living room part 0 is "You're now in a living room. You decide to start listing off everything you see in the room, as if you were in a text adventure.

 You can see a couch. [if there is something on the s_5]On the couch you see [a list of things on the s_5].[end if]".
...
The living room part 2 is some text that varies. The living room part 2 is " Look out! It's a- oh, never mind, it's just a low table. The low table is normal.[if there is something on the s_6] On the low table you can see [a list of things on the s_6]. There's something strange about this being here, but you can't put your finger on it.[end if]".
The living room part 3 is some text that varies. The living room part 3 is "[if there is nothing on the s_6] But oh no! there's nothing on this piece of trash.[end if]".
The living room part 4 is some text that varies. The living room part 4 is " You scan the room for a tv, and you find a tv. The tv is standard.[if there is something on the s_7] On the tv you make out [a list of things on the s_7].[end if]".
...
The bathroom part 2 is some text that varies. The bathroom part 2 is " You bend down to tie your shoe. When you stand up, you notice a bath. Something scurries by right in the corner of your eye. Probably nothing.[if c_4 is open and there is something in the c_4] The bath contains [a list of things in the c_4].[end if]".
...
The bathroom part 4 is some text that varies. The bathroom part 4 is " Were you looking for a sink? Because look over there, it's a sink. [if there is something on the s_4]You see [a list of things on the s_4] on the sink.[end if]".
The bathroom part 5 is some text that varies. The bathroom part 5 is "[if there is nothing on the s_4]Looks like someone's already been here and taken everything off it, though.[end if]".
...
The c_4 is in r_3.
The c_4 is closed.
The description of f_3 is "that's an ordinary apple!".
The printed name of f_3 is "apple".
Understand "apple" as f_3.
The f_3 is in r_5.
The f_3 is edible.
The description of f_4 is "that's a normal lettuce!".
The printed name of f_4 is "lettuce".
Understand "lettuce" as f_4.
The f_4 is in r_5.
The f_4 is edible.
The description of f_5 is "The bell pepper looks delectable.".
...
The description of f_1 is "that's a normal milk!".
The printed name of f_1 is "milk".
Understand "milk" as f_1.
The f_1 is edible.
The f_1 is in the c_2.
The description of f_2 is "that's an usual half of a bag of chips!".
...
The description of o_2 is "The remote looks to fit in here".
...
The objective part 0 is some text that varies. The objective part 0 is "Welcome to TextWorld! Here is how to play! First off, make it so that the chest drawer is ajar. And then, retrieve the old key from the chest drawer within the bedroom. And then, assure that the woode".
The objective part 1 is some text that varies. The objective part 1 is "n door within the bedroom is unlocked with the old key. Then, ensure that the wooden door is open. After opening the wooden door, make an effort to take a trip east. After that, open the screen door i".
The objective part 2 is some text that varies. The objective part 2 is "nside the kitchen. After that, make an effort to take a trip east. Then, move south. With that over with, lift the lettuce from the floor of the garden. After that, go to the north. Next, go west. The".
The objective part 3 is some text that varies. The objective part 3 is "n, rest the lettuce on the stove. Alright, thanks!".
